...
test.describe("check test workspace members with random member check", () => {
  test.describe.configure({ mode: "serial" });
  let memberId: string;
...
test("test workspace create secret key", async ({ request }) => {
  expect(TEST_WORKSPACE_ID).toBeTruthy();
...
test.describe("create workspace with labels flow ", () => {
  test.describe.configure({ mode: "serial" });
  ...
  test("create workspace label", async ({ request }) => {
    const workspaceLabelName = faker.hacker.noun();
    const data = {
      name: workspaceLabelName,
    };
    const response = await request.post(`/workspaces/${workspaceId}/labels/`, {
      data,
    });
    expect(response.status()).toBe(201);
    ...
    expect(body.name).toBe(workspaceLabelName);
    expect(body.createdAt).toBeTruthy();
    expect(body.updatedAt).toBeTruthy();
  ...
  test("check list of labels that were created", async ({ request }) => {
    const response = await request.get(`/workspaces/${workspaceId}/labels/`);
    expect(response.ok()).toBeTruthy();
    expect(response.status()).toBe(200);
    ...
    for (const label of body) {
      ...
      if (label.name === labelName && label.labelId === labelId) {
        hasWorkspaceLabel = true;
      }
    }

    expect(hasWorkspaceLabel).toBe(true);
  });

  test("update the workspace label", async ({ request }) => {
    const updatedName = faker.hacker.noun();
    const data = {
      name: updatedName,
      icon: "🚀",
    };
  ...
  test("check the updated workspace label", async ({ request }) => {
    const response = await request.get(
      `/workspaces/${workspaceId}/labels/${labelId}/`
    );
    expect(response.status()).toBe(200);
